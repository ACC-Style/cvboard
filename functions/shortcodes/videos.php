...
function codewp_render_custom_posts_videos($atts) {
    ...
    if($atts['count'] >= 0 ){
        display_videos_by_count($atts['category'], $atts['count'], $atts['cols_large'], $atts['cols_medium']);
    }else{
...
function display_videos_by_count($category_slug, $count, $cols_large, $cols_medium, $show_category_name = true) 
    {
    // Query parameters
    $args = array(
        'post_type' => 'videos',
